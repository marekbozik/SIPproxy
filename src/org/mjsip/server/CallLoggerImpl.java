...
public class CallLoggerImpl implements CallLogger {
    ...
    public void update(SipMessage msg) {
        ...
        try {
            ...
            //java.util.logging.Logger.getLogger(CallLoggerImpl.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void calllog(String call_id) {
        ...
        if (invite_time != null && accepted_time != null && bye_time != null) //call_logger.log(DateFormat.formatHHMMSS(invite_time)+"\t"+call_id+"\tCALL \t"+callers.get(call_id)+"\t"+callees.get(call_id)+"\t"+(accepted_time.getTime()-invite_time.getTime())+"\t"+(bye_time.getTime()-accepted_time.getTime()));
        {
